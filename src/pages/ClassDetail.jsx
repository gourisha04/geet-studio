...
export default function ClassDetail() {
    ...
    api.get(`/api/classes/${id}`)
      .then((res) => {
        if (!cancelled && res?.success && res.data) {
          setClassData(normalizeClassRecord(res.data));
        }
      })
      .catch(() => {});
  ...
  const [regForm, setRegForm] = useState({ name: '', email: '', phone: '', age: '', batch: '', message: '' });
  ...
  const handleRegSubmit = async (e) => {
    ...
    try {
      ...
      if (!data?.success) {
        throw new Error(data?.message || 'Unable to submit enrollment request.');
      }

      setRegSubmitted(true);
    } catch (err) {
      setErrorMsg(err.message || 'Unable to submit enrollment request. Please try again.');
    } finally {
      setRegLoading(false);
    }
  };
          ...
          >
            ...
            {regSubmitted ? (
              <div className="text-center py-8">
                ...
                <h3 className="font-heading text-2xl font-bold mb-2">Enrollment Request Received!</h3>
                <p className="text-sm opacity-80 mb-2">
                  Your request to join <strong className="text-gold-500">{classData.name}</strong> has been saved.
                </p>
              ...
              <form onSubmit={handleRegSubmit} className="space-y-4">
                <div>
                  <p className="text-xs uppercase text-gold-500 font-semibold tracking-wider">Join Class Request</p>
                ...
                <div>
                  ...
                  <textarea rows={2} value={regForm.message} onChange={(e) => setRegForm({ ...regForm, message: e.target.value })} placeholder="Any message or questions for the studio..." className={`w-full p-2.5 text-sm rounded border ${isDark ? 'bg-dark-800 border-dark-700' : 'bg-warm-50 border-warm-300'}`} />
                </div>
                ...
                <button type="submit" disabled={regLoading} className="w-full py-3 bg-gold-500 text-dark-950 font-bold text-xs uppercase tracking-widest hover:bg-gold-400 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg">
                  {regLoading ? 'Submitting...' : 'Submit Join Class Request'}
                  <Send className="w-4 h-4" />
                </button>
              </form>
            )}
